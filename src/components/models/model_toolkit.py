    ...
    **kwargs :
    ...
    if numeric_features:
        ...
        if missing_strategy != "none" and numeric_strategy != "none":
            ...
            num_pipeline = Pipeline(
                [("imputer", SimpleImputer(strategy=missing_strategy, **imputer_kwargs)), ("scaler", num_transformer)]
            )
    ...
    if categorical_features:
        if categorical_strategy == "onehot":
            ...
            raise ValueError(f"Unknown categorical strategy: {categorical_strategy}") from None

        # Handle missing values for categorical features
        if missing_strategy != "none" and categorical_strategy != "none":
            cat_pipeline = Pipeline(
                [("imputer", SimpleImputer(strategy="most_frequent")), ("encoder", cat_transformer)]
            )
    ...
    if task == "classification":
        if model_type == "random_forest":
            ...
            try:
                ...
                raise ImportError("LightGBM is not installed. Install with 'pip install lightgbm'") from None
            ...
            try:
                ...
                raise ImportError("XGBoost is not installed. Install with 'pip install xgboost'") from None
            ...
            try:
                ...
                raise ImportError("CatBoost is not installed. Install with 'pip install catboost'") from None
        else:
            raise ValueError(f"Unknown classification model type: {model_type}") from None
        ...
        raise ValueError(f"Unknown task type: {task}. Must be 'classification' or 'regression'") from None
    ...
    if resampling_strategy and task == "classification":
        try:
            ...
            if resampling_strategy == "smote":
                ...
                raise ValueError(f"Unknown resampling strategy: {resampling_strategy}") from None
    ...
    try:
        ...
        # Additional metrics for regression
            ...
            try:
                ...
                logger.info(
                    f"Validation metrics: MSE={metrics['mse']:.4f}, "
...
def register_mlflow_model(
    ...
    # Register the model
    registered_model = mlflow.register_model(model_uri=f"runs:/{run_id}/{artifact_path}", name=model_name, tags=tags)
    ...
    return {"name": model_name, "version": latest_version, "alias": alias, "run_id": run_id}


def batch_score_model(
    spark, model_uri: str, data: Union[pd.DataFrame, "SparkDataFrame"], feature_engineering: bool = True, **kwargs
    ...
    **kwargs :
    ...
    if feature_engineering:
        try:
            ...
            # Score batch
            predictions = fe_client.score_batch(model_uri=model_uri, df=spark_df, **kwargs)
    ...
    logger.info(
        f"Batch scoring completed with {predictions.shape[0] if is_pandas else predictions.count()} predictions"
    )
    return predictions
...
def compare_models(
    ...
    champion_preds = batch_score_model(
        spark=spark, model_uri=champion_model_uri, data=test_data, feature_engineering=feature_engineering
    )

    challenger_preds = batch_score_model(
        spark=spark, model_uri=challenger_model_uri, data=test_data, feature_engineering=feature_engineering
    )
    ...
    if metric == "roc_auc":
        ...
        raise ValueError(
            f"Unsupported metric: {metric}. Supported metrics: roc_auc, accuracy, f1, precision, recall"
        ) from None
    ...
    logger.info(
        f"Model comparison completed: Champion {metric}={champion_score:.4f}, "
    ...
    try:
        ...
        try:
            ...
            if metric == "roc_auc":
                ...
                raise ValueError(f"Unsupported metric: {metric}") from None
            ...
            # Log results
            logger.info(
                f"Model comparison: Current model {metric}={current_score:.4f}, "
                f"Latest model {metric}={latest_score:.4f}"
            )

            if model_improved:
                logger.info(
                    f"Current model performs better (improvement: {current_score - latest_score:.4f}). "
                    f"Register the challenger."
                )
            else:
                logger.info("Current model does not show sufficient improvement. " "Keep the champion model.")
